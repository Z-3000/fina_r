...
App.jsx의 대형 컴포넌트들을 개별 파일로 분리하여 유지보수성을 향상시켰습니다.

| 파일 | 용도 | 위치 |
|------|------|------|
| `DashboardView.jsx` | 대시보드 탭 전체 | `src/components/views/` |
| `BudgetView.jsx` | 예산 관리 탭 | `src/components/views/` |
| `TaxPredictionView.jsx` | 세금예측 탭 | `src/components/views/` |

### 대시보드 UI 개선

#### 1. 세금 현황 카드 스타일 개선
- **적용 대상**: 개인 연말정산 현황 + 사업자 세금 현황
- **변경**: 카드 하단 설명 텍스트 `text-right` 추가 (오른쪽 정렬)

#### 2. 카테고리별 지출 원형차트 개선

| 항목 | 변경 전 | 변경 후 |
|------|---------|---------|
| 10% 미만 항목 | 개별 표시 | "기타"로 통합 |
| 색상 | 기본 팔레트 (중복 가능) | 11가지 고유 색상 팔레트 |

**새 색상 팔레트 (UNIQUE_CHART_COLORS)**:
```javascript
['#6366F1', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6',
 '#06B6D4', '#EC4899', '#84CC16', '#F97316', '#14B8A6', '#9CA3AF']
```
...
| 예상 세금 (막대) | 예상 세금 |
| 누적 세금 (라인) | 누적 세금 |
| 공제 미적용 시 (라인) | 공제 미적용 누적 |

#### 2. 공제 미적용 라인차트 누적값 적용
- **목적**: 누적 세금 라인과 직관적 비교 가능
- **변경**: `noDeductionTax` → `noDeductionCumulative`
- **Y축**: right (누적 세금과 동일 스케일)

### 월별 지출 추이 차트 DB 연동

#### 문제점
기존 `monthlySpendingData`는 예산을 현재 월 값으로 고정하여 모든 월에 동일하게 표시됨

#### 해결 방법
1. **API 추가** (`supabaseApi.js:225-282`)
   - `budgetsAPI.getMonthlySpendingTrend(userId)` 함수 추가
   - receipts + budgets 테이블에서 최근 6개월 월별 집계

2. **App.jsx 수정**
   - `monthlySpendingTrendData` state 추가
   - `loadDataFromAPI`에서 API 호출
   - `monthlySpendingData` useMemo에서 DB 데이터 우선 사용

#### DB 연동 결과 (실제 데이터)
| 월 | 지출 | 예산 |
|----|------|------|
| 6월 | 13,085,741원 | 14,180,000원 |
| 7월 | 12,607,166원 | 14,180,000원 |
| 8월 | 13,061,730원 | 14,180,000원 |
| 9월 | 12,563,108원 | 14,180,000원 |
| 10월 | 12,319,432원 | 14,180,000원 |
| 11월 | 12,488,402원 | 14,180,000원 |

### 수정된 파일 요약

| 파일 | 변경 내용 |
|------|----------|
| `src/components/views/DashboardView.jsx` | 세금 현황 카드 정렬, 원형차트 기타 처리 + 고유 색상 |
| `src/components/views/TaxPredictionView.jsx` | 범례 텍스트 간소화, 누적 라인차트 |
| `src/services/supabaseApi.js` | `getMonthlySpendingTrend` API 추가 |
| `src/App.jsx` | `monthlySpendingTrendData` state, DB 연동 |
